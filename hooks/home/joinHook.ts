  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const getHomeResponse = await fetch(
        `${API_ENDPOINTS.HOME_SEARCH}?auth_id=${homeId}&auth_token=${homePassword}`,
        {
          headers: {
      ...
      if (!getHomeResponse.ok || !homeData?.id) {
        Toast.show({
          type: "error",
      ...
      const propertyId = homeData.id;

      // Set is_default=false for all other homes of this user
      await fetch(
        `${API_ENDPOINTS.PROPERTY_USERS}?filter[directus_users_id][_eq]=${userId}&filter[properties_id][_neq]=${propertyId}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
